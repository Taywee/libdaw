...
use libdaw::pitch::{PitchStandard as DawPitchStandard, A440 as DawA440};
...
pub struct PitchStandard(pub Arc<dyn libdaw::pitch::PitchStandard>);

#[pymethods]
impl PitchStandard {
    pub fn resolve(&self, pitch: &Bound<'_, Pitch>) -> f64 {
        self.0.resolve(pitch.borrow().0)
    }
...
pub struct A440(pub Arc<dyn libdaw::pitch::PitchStandard>);

#[pymethods]
impl A440 {
    #[new]
    pub fn new() -> PyClassInitializer<Self> {
        let inner = Arc::new(libdaw::pitch::A440);
        PyClassInitializer::from(PitchStandard(inner.clone())).add_subclass(Self(inner))
    }
}

#[pyclass(extends = PitchStandard, subclass, module = "libdaw.pitch")]
pub struct ScientificPitch(pub Arc<dyn libdaw::pitch::PitchStandard>);

#[pymethods]
impl ScientificPitch {
    #[new]
    pub fn new() -> PyClassInitializer<Self> {
        let inner = Arc::new(libdaw::pitch::ScientificPitch);
        PyClassInitializer::from(PitchStandard(inner.clone())).add_subclass(Self(inner))
    }
}
